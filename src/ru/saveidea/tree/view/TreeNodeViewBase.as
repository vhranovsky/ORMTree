package ru.saveidea.tree.view {
	...
	public class TreeNodeViewBase extends Sprite {
		...
		protected function onTreeNodeViewSelectHandler(event : TreeNodeViewBaseEvent) : void {
			if (selectedNodeView) {
				selectedNodeView.deselect();
			}
			
			selectedNodeView = event.nodeView;
		}
		...
		public function select() : void {
			getRootView().dispatchEvent(new TreeNodeViewBaseEvent(TreeNodeViewBaseEvent.SELECT, this));
		...
		protected function getSkinClass() : Class {
			return TreeNodeSkinDefault;
		}

		private function onUpdateViewHandler(event : TreeNodeViewBaseEvent) : void {
			update();
			dispatchEvent(new TreeNodeViewBaseEvent(TreeNodeViewBaseEvent.UPDATE_VIEW, this));
		}

		private function onUpdateHandler(event : TreeNodeViewBaseEvent) : void {
			if (parentNodeView) {
				dispatchEvent(new TreeNodeViewBaseEvent(TreeNodeViewBaseEvent.UPDATE_DATA, this));
			} else {
				// model = _model;
